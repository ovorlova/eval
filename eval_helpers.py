...
class Joint:
    def __init__(self):
        ...
        self.name[self.throax]      = "throax"
        self.name[self.upper_neck]       = "upper_neck"
        ...
        self.symmetric_joint[self.throax]           = -1
        self.symmetric_joint[self.head_top]       = -1
        self.symmetric_joint[self.upper_neck]       = -1
...
def turnToPCKhDict(pckAll):
    names = ['r_ankle', 'r_knee', 'r_hip', 'l_hip', 'l_knee', 'l_ankle', 'pelvis', 'throax', 'upper_neck', 'head_top', 'r_wrist', 'r_elbow', 'r_shoulder', 'l_shoulder', 'l_elbow', 'l_wrist', 'total']
    pckh = [pckAll[i][0] for i in range(len(pckAll))]
...
def printResults(dct):
    for key in dct:
      if len(key) < 7:
        print(key, ':\t\t', float('{:.2f}'.format(dct[key])), sep='')
      else:
        print(key, ':\t', float('{:.2f}'.format(dct[key])), sep='')
    print()
